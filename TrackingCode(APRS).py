...
ensembles = 50
errInEnsembles = 0.2
approxLaunchAlt = 1000
reqPred = 10000
tolerace = 3000                 # [ft] How much of a change warrents a message to the group
...
while tracking == True:
    ...
    if (time.time()-LastAPRSCheck) > APRSfreq:
        ...
        try:
            ...
            data = balloon.prediction(payload,balloonWeight,parachuteDia,helium,APRS_data['lat'],APRS_data['lng'],APRS_data['altitude'] * 3.281,status,'now',ensembles,errInEnsembles)
            ...
            if errorNum == 1:
                message = 'ALERT: Could not recieve APRS data. Attempting to fix...'
                balloon.send_slack(message,messageType,recipient,slackURL)
                time.sleep(10)
            if errorNum > 5:
        ...
        print('Done with 30 second chunk')
